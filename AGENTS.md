...
### 4.2. Star Schema Conventions
- **Denormalized Dimensions**: Place dimensional marts in `dbt/models/marts/`. Build **Star Schemas** where central fact tables join directly to flat, denormalized dimensions in a single hop.
...
- **Core Marts (`dbt/models/marts/`)**: Default core dimension models (`dim_<entity>.sql`) to clean SCD Type 1 materializations (`+materialized: table`) for performant standard BI joins.
...
8. **Follow Kimball Star Schema & Hybrid SCD Principles**: Model marts as star schemas with declared atomic grains and surrogate keys (`<entity>_sk`). Assign SCD1 (overwrite) vs. SCD2 (validity tracking) on an attribute-by-attribute basis; do not blanket-apply SCD2 across entire dimensions.
